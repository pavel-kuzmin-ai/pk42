...
void pk42Core::runGameLoop()
{
	...
	while (sEngineState.bEngineInitialized)
	{
		...
		{
			position += 1;
			position = position % display->bufSize();
		}
		step();
...
void pk42Core::step()
{
	...
	//{
		...
		colorBuf[(position + 1)% display->bufSize()] = colorRGB(255, 0, 0);
		colorBuf[(position + 2) % display->bufSize()] = colorRGB(0, 255, 0);
		colorBuf[(position + 3) % display->bufSize()] = colorRGB(0, 0, 255);
		colorBuf[(position + 4) % display->bufSize()] = colorRGB(255, 255, 0);
		colorBuf[(position + 5) % display->bufSize()] = colorRGB(0, 255, 255);
		colorBuf[(position + 6) % display->bufSize()] = colorRGB(255, 0, 255);
		colorBuf[(position + 7) % display->bufSize()] = colorRGB(255, 255, 255);
		colorBuf[(position + 8) % display->bufSize()] = colorRGB(128, 32, 64);
		colorBuf[(position + 9) % display->bufSize()] = colorRGB(65, 92, 12);
	//}
